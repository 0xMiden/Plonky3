...
fn reduce192(lo: u64, hi: u64, carry: u64) -> Goldilocks {
    const EPS2_MOD_P: u64 = 0xFFFF_FFFF_0000_0001u64.wrapping_sub(1 << 32); // ε² mod P
...
fn mul(x: uint64x2_t, y: uint64x2_t) -> uint64x2_t {
    unsafe {
        use core::arch::{
            aarch64::{vcgtq_u64, vmlal_n_u32, vmovn_u64, vsraq_n_u64},
            asm,
        };
        let lo: uint64x2_t;
        let hi: uint64x2_t;
...
mod mixed_dot_tests {
    ...
    fn dot_ref<const N: usize>(a: &[PackedGoldilocksNeon; N], f: &[Goldilocks; N], lane: usize) -> u64 {
        let mut acc: u128 = 0;
        for i in 0..N {
    ...
    fn check_mixed_dot<const N: usize>(a_raw: &dyn Fn(usize, usize) -> u64, f_raw: &dyn Fn(usize) -> u64) {
    ...
    fn reduce192_matches_reference() {
        ...
        for &(lo, hi, carry) in
            &[(0, 0, 0), (u64::MAX, u64::MAX, (1 << 32) - 2), (P, P, 1), (u64::MAX, 0, 0)]
        {
            assert_eq!(reduce192(lo, hi, carry).as_canonical_u64(), reference(lo, hi, carry));
        }
    }
}
